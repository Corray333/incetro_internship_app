      ...
      <section v-for="(section, i) of todo" :key="i" class=" flex flex-col gap-2">
        ...
        <div class="flex gap-2 overflow-x-scroll">
          <article v-for="(task, j) of section" :key="j" @click="openTask(task)"
            ...
            <p v-if="task.status != 'Не начато'" class=" flex justify-center items-center w-full p-2 absolute bottom-0 left-0 rounded-md text-white bg-accent">{{ task.status }}</p>
            <div>
